...
Point getSquareCenter(const vector<Point>& square)
{
    Point center;
...
void filterDuplicitSquares( vector<vector<Point> >& squares )
{
    for (int i = 0; i < (int)squares.size()-1; i++)
    {
        ...
        {
            if (fabs(getSquareCenter(squares[i]).x - getSquareCenter(squares[j]).x) < FILTERING_WINDOW)
            {
                if (fabs(getSquareCenter(squares[i]).y - getSquareCenter(squares[j]).y) < FILTERING_WINDOW)
                {
